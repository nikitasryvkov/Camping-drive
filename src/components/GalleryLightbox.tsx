...
export default function GalleryLightbox({ index, onChange, onClose }: { index: number | null; onChange: (index: number) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft") onChange((index - 1 + GALLERY.length) % GALLERY.length);
